...
// Negative = AI-default tells. Lower is better.
const NEGATIVE = [
  ["default_blue_3B82F6", /#3b82f6\b/gi],
  ...
  ["inter_font", /font-family[^;{}]*\bInter\b/gi],
...
function walk(dir, files = []) {
  for (const name of readdirSync(dir)) {
    ...
    else if (CODE_EXT.has(extname(name).toLowerCase())) files.push(p);
  }
  return files;
}

function countGroups(texts, groups) {
  const out = {};
...
export function scanDir(dir) {
  ...
  return {
    ...
    negatives: countGroups(texts, NEGATIVE),
    positives: countGroups(texts, POSITIVE),
    neutral: countGroups(texts, NEUTRAL),
  };
}
...
if (isCLI) {
  ...
  console.log(`Scanned ${r.fileCount} code files in ${r.dir}`);
